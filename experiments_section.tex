...
\subsubsection{训练平台与运行记录}

Wave-U-Net在Linux服务器上使用Python~3.11、PyTorch~2.5.1、CUDA~12.4和
RTX~4090完成训练，模型包含24,297,913个可训练参数。Edinburgh使用3张GPU，
训练100轮，最佳模型来自第89轮，验证$L_1$为0.005983，墙钟窗口约3小时42分钟。
MS-SNSD使用4张GPU，最佳模型来自第77轮，验证$L_1$为0.002729，并在第97轮因
连续20轮未改善而提前停止，总耗时约5小时。测试均只加载验证损失最低的检查点。
...
指标。需要注意，Wave-U-Net的MS-SNSD实验采用从\texttt{clean\_train}和
\texttt{noise\_train}统一划分得到的同域测试集，并保证训练、验证、测试间
说话人与噪声文件互斥；RNNoise仍使用官方1,100条测试集，因此两行不能作为
严格的同测试集横向比较。
...
& Edinburgh & 824 & 19.592 & 10.611 & 19.539 & 2.522 & 0.935 \\
& MS-SNSD（同域） & 682 & 25.704 & 17.680 & 25.647 & 3.018 & 0.872 \\
\bottomrule
\end{tabular}}}
\end{table*}

Edinburgh使用相同的824条官方测试语音，Wave-U-Net在五项指标上均高于本实验的
RNNoise结果。MS-SNSD的Wave-U-Net含噪输入基线为SNR~19.967~dB、
SSNR~13.785~dB、SI-SDR~19.968~dB、PESQ~2.450和STOI~0.841；增强后的对应
增益为$+5.738$~dB、$+3.896$~dB、$+5.679$~dB、$+0.567$和$+0.031$。
由于两种方法在MS-SNSD上使用不同测试集合，该数据集仅报告各自绝对结果，
不据此判断方法间优劣。
