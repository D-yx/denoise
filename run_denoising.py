...
def paired_files(dataset: str, split: str) -> list[tuple[Path, Path]]:
    ...
    made = ROOT / ".prepared" / "ms-snsd" / split
    return [(made / "noisy" / p.name, p) for p in sorted((made / "clean").glob("*.wav"))
            if (made / "noisy" / p.name).exists()]


def preprocess_ms(seed: int, force: bool) -> None:
    base, out = DATASETS / "MS-SNSD", ROOT / ".prepared" / "ms-snsd"
    rng = np.random.default_rng(seed)
    for split in ("train", "test"):
        cleans = sorted((base / f"clean_{split}").glob("*.wav"))
        noises = sorted((base / f"noise_{split}").glob("*.wav"))
        if not cleans or not noises:
            raise FileNotFoundError(f"MS-SNSD clean_{split}/noise_{split} is incomplete")
        log(f"MS-SNSD {split}: synthesising {len(cleans)} noisy-clean pairs")
        for i, cp in enumerate(cleans, 1):
            dst_c, dst_n = out / split / "clean" / cp.name, out / split / "noisy" / cp.name
            if force or not (dst_c.exists() and dst_n.exists()):
                c = audio(cp, 16000)
                n = audio(noises[int(rng.integers(len(noises)))], 16000)
                snr = float(rng.choice([0, 10, 20, 30, 40]))
                write(dst_c, c, 16000); write(dst_n, mix_at_snr(c, n, snr, rng), 16000)
            if i % 250 == 0 or i == len(cleans): log(f"  {i}/{len(cleans)}")
...
class PairDataset:
    ...
    def __getitem__(self, idx):
        noisy_p, clean_p = self.pairs[idx]; x, y = audio(noisy_p, self.sr), audio(clean_p, self.sr)
...
def train_wave(dataset: str, args: argparse.Namespace) -> None:
    if dataset == "ms-snsd" and not paired_files(dataset, "train"): preprocess_ms(args.seed, args.force)
    torch, Waveunet = wave_imports(); torch.manual_seed(args.seed); random.seed(args.seed)
    device = torch.device("cuda" if not args.cpu and torch.cuda.is_available() else "cpu")
    log(f"Wave-U-Net training device: {device}" + (f" ({torch.cuda.get_device_name(0)})" if device.type == "cuda" else ""))
    model = make_wave_model(torch, Waveunet, args.wave_sr).to(device)
    pairs = paired_files(dataset, "train"); rng = random.Random(args.seed); rng.shuffle(pairs)
    cut = max(1, int(len(pairs) * .9)); train, val = pairs[:cut], pairs[cut:] or pairs[-1:]
    from torch.utils.data import DataLoader
    tr = DataLoader(PairDataset(train, model, args.wave_sr, True), batch_size=args.wave_batch_size,
                    shuffle=True, num_workers=args.workers)
    va = DataLoader(PairDataset(val, model, args.wave_sr, False), batch_size=args.wave_batch_size,
                    num_workers=args.workers)
    out = ROOT / ".prepared" / f"wave-u-net-{dataset}"; out.mkdir(parents=True, exist_ok=True)
    ...
    if last_path.exists() and not args.no_resume:
        ...
        log(f"Resuming Wave-U-Net from epoch {start_epoch-1}; best_val_loss={best:.6f}")
    if start_epoch > args.wave_epochs:
        log(f"Wave-U-Net training already reached {args.wave_epochs} epochs")
        return
    if bad >= args.patience:
        log(f"Wave-U-Net already stopped after {bad} unimproved epochs")
        return
    for epoch in range(start_epoch, args.wave_epochs + 1):
        model.train(); total = 0.0
        for step, (x, y) in enumerate(tr, 1):
            x, y = x.to(device), y.to(device); opt.zero_grad(); pred = model(x)["clean"]
            loss = loss_fn(pred, y); loss.backward(); opt.step(); total += loss.item()
            if step % args.log_interval == 0 or step == len(tr): log(f"epoch {epoch} {step}/{len(tr)} loss={total/step:.6f}")
        model.eval(); v = 0.0
        with torch.no_grad():
            for x, y in va: v += loss_fn(model(x.to(device))["clean"], y.to(device)).item()
        v /= max(1, len(va)); log(f"epoch {epoch}: train_loss={total/max(1,len(tr)):.6f} val_loss={v:.6f}")
        improved = v < best
        ...
        if bad >= args.patience: log(f"Early stopping after {bad} unimproved epochs"); break


def enhance_wave(path: Path, checkpoint: Path, sr: int, cuda: bool) -> tuple[np.ndarray, int]:
...
def test(method: str, dataset: str, args: argparse.Namespace) -> None:
    ...
    pairs = paired_files(dataset, "test"); out = RESULTS / f"{method}_{dataset}"
    ...
    for i,(noisy,clean_p) in enumerate(pairs,1):
        if method == "hybrid":
            ...
            enhanced,sr=enhance_wave(noisy,ROOT/".prepared"/f"wave-u-net-{dataset}"/"best.pt",args.wave_sr,not args.cpu)
        clean=audio(clean_p,sr); values=metric_values(clean,enhanced,sr); values["file"]=clean_p.name; rows.append(values)
...
def main(fixed_method: str | None=None, fixed_dataset: str | None=None) -> None:
    ...
    if args.stage in ("preprocess","all"):
        (prepare_hybrid(dataset,args) if method=="hybrid" else (preprocess_ms(args.seed,args.force) if dataset=="ms-snsd" else log("Edinburgh pairs already need no conversion")))
