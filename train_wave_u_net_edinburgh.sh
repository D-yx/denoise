...
mapfile -t selected_gpus < <(
    ...
    awk -F', *' '
        ...
        END {
            for (gpu in count) {
                average_util=util[gpu]/count[gpu]
                average_memory=memory[gpu]/count[gpu]
...
cache_marker=".prepared/wave-u-net-edinburgh-16000hz/train/.complete"
if [[ ! -f "${cache_marker}" ]]; then
    echo "Prepared 16 kHz cache not found. Run ./preprocess_wave_u_net_edinburgh.sh first." >&2
    exit 1
fi

exec env CUDA_VISIBLE_DEVICES="${gpu_ids}" conda run --no-capture-output -n denoise \
    torchrun --standalone --nproc-per-node="${gpu_count}" \
    wave_u_net_edinburgh.py --stage train "$@"
